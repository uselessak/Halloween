...
// 封装函数来处理导航栏事件
function setupMoreGamesNavEvents() {
    const moreGamesNav = document.querySelector('.more-games-nav');
    if (moreGamesNav) {
        moreGamesNav.addEventListener('mouseenter', function () {
            console.log('鼠标进入导航栏');
            this.classList.add('expanded');
        });
        moreGamesNav.addEventListener('mouseleave', function () {
            console.log('鼠标离开导航栏');
...
// 初始化导航栏事件
setupMoreGamesNavEvents();

// 为了确保在页面结构变化后导航栏事件依然有效，我们可以监听 DOM 变化
const observer = new MutationObserver(() => {
    // 移除之前的事件监听器（简单模拟，实际中需要更完善的处理）
    const moreGamesNav = document.querySelector('.more-games-nav');
    if (moreGamesNav) {
        moreGamesNav.removeEventListener('mouseenter', null);
        moreGamesNav.removeEventListener('mouseleave', null);
        // 重新设置事件监听器
        setupMoreGamesNavEvents();
    }
});
